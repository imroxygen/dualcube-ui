...
const CatalogCustomizer: React.FC<CatalogCustomizerProps> = ({ onChange, proSetting, setting, Sample_Product, pro_url }) => {
...
const [dragableItems, setDragableItems] = useState<DraggableItem[]>([
  {
    id: 'price_section',
    content: () => {
      const [hideProductPrice, setHideProductPrice] = useState<boolean>(setting['hide_product_price']);
      return (
        <div className='price-section toggle-visibility'>
          ...
          <p className='product-price' style={{ opacity: hideProductPrice ? "0.3" : "1" }}>
  ...
  },
  {
    ...
    content: () => {
      const [hideProductDesc, setHideProductDesc] = useState<boolean>(setting['hide_product_desc']);
      return (
        <div className='description-section toggle-visibility'>
          ...
          <p className='product-description' style={{ opacity: hideProductDesc ? "0.3" : "1" }}>
  ...
  },
  {
    ...
    content: () => (
      <section className='catalog-add-to-cart-section'>
        ...
        <div className='main-btn btn-purple catalog-add-to-cart-btn'>Add to cart</div>
      </section>
    ),
    defaultPosition: 3,
    dragable: false,
 ...
 */
const getIndex = <T extends { id: string }>(list: T[], id: string): number => {
...
useEffect(() => {
  ...
  let positionSetting: Record<string, string> = shopPagePossitionSetting || {};
...
useEffect(() => {
  setButtonItems((prevButtonItems) => {
    return [...prevButtonItems].sort(
      (a, b) => buttonPossitionSetting.indexOf(a.id) - buttonPossitionSetting.indexOf(b.id)
    );
  });
}, []);
 ...
 */
const handleSubMenuChange = (newTab: { id: string; name: string; icon:string }): void => {
      ...
      <main className="catelog-customizer-main-wrapper" id="catelog-customizer-main-wrapper">
          ...
          <div className="product-data">
            ...
            <div className="drag-drop-component">
              ...
              >
                {dragableItems.map((item, index) => (
                  <div key={index} className={`${item.dragable ? "should-move" : ""} shop-page-draggable`}>
                    {item.content === "buttonDND" ? (
                      <div className="button-wrapper">
                        ...
                        >
                          {buttonItems.map((btn) => (
                            <div key={btn.id} className="shop-page-button-draggable">
                              {btn.id === "enquiry_button" && (
                                ...
                                >
                                  <ButtonCustomizer
                                    ...
                                    text={localSetting?.enquiry_button?.button_text || "Enquiry"}
                                    setting={localSetting?.enquiry_button}
                                    onChange={(key, value, isRestoreDefaults = false) => {
                                      setSetting("enquiry_button", isRestoreDefaults ? value : { ...localSetting.enquiry_button, [key]: value });
                                    }}
                                  />
                                </div>
                              )}
                              {btn.id === "cart_button" && (
                                <ButtonCustomizer
                                  ...
                                  onChange={(key, value, isRestoreDefaults = false) => {
                                    setSetting("cart_button", isRestoreDefaults ? value : { ...localSetting.cart_button, [key]: value });
                                  }}
                                />
                              )}
                              {btn.id === "quote_button" && (
                                ...
                                >
                                  <ButtonCustomizer
                                    text={localSetting?.quote_button?.button_text || "Add to quote"}
                                    setting={localSetting?.quote_button}
                                    onChange={(key, value, isRestoreDefaults = false) => {
                                      setSetting("quote_button", isRestoreDefaults ? value : { ...localSetting.quote_button, [key]: value });
                                    }}
                                  />
                                </div>
                              )}
                      ...
                      >
                        <input
                          ...
                          onChange={(e) => setSetting("additional_input", e.target.value)}
                        />
                      </div>
                    ) : (
                      typeof item.content === "function" && React.createElement(item.content, { currentTab, setCurrentTab })
                    )}
                  </div>
                ))}
              </ReactDragListView>
            ...
            {!proSetting && (
              <article className="pro-banner">
                ...
                <a href={pro_url} className="main-btn btn-purple" target="_blank" rel="noopener noreferrer">
                  Upgrade now
                </a>
              </article>
            )}
        ...
        <section className="single-product-page-description">
          <div className="option">
            <ul>
              <li className="active">
                Description <span><i className="admin-font adminLib-keyboard-arrow-down"></i></span>
              </li>
